...
class MediathekCockpit(Screen, Menu, ChannelSelection, Search):
    ...
    def __onLayoutFinish(self):
        ...
        if listbox and listbox.instance:
            listbox.instance.allowNativeKeys(False)
        if self.query:
            query = self.query
            self.query = ""
    ...
    def pressYellow(self):
        ...
        curr = self.movie_list.getCurrentSelection()
        if curr:
            url, recording_path = self.movie_file.download(curr, self.job_manager)
            if url and recording_path:
                self.session.open(
                    MessageBox,
    ...
    def pressInfo(self):
        curr = self.movie_list.getCurrentSelection()
        if curr:
            self.session.open(EventInfo, curr)
